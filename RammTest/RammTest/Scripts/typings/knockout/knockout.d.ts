...
interface KnockoutSubscriptionChangedItem<T> {
    value: T;
    index: number;
    status: string;
}
...
interface KnockoutObservableArray<T> extends KnockoutObservableArrayFunctions<T> {
    ...
    (value: T[]): void;
    subscribe(callback: (newValue: T[]) => void, target?: any, topic?: string): KnockoutSubscription;
    subscribe(callback: (newValue: KnockoutSubscriptionChangedItem<T>[]) => void , target?: any, topic?: string): KnockoutSubscription;
    notifySubscribers(valueToWrite: T[], topic?: string);
}
...
interface KnockoutStatic {
    ...
    pureComputed: KnockoutComputedStatic;
